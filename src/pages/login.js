...
const Login = ({ onSwitch }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const csrfRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL_PAYMENT}/admin/csrf-token`, {
        credentials: 'include',
      });
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL_PAYMENT}/admin/login`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        login(result); // ✅ Save user to context
